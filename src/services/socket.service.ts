    ...
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on('device:register', async (deviceInfo) => {
        try {
          ...
          // Default to localhost if IP is not valid IPv4
          if (!clientIp || clientIp === '::1' || !isValidIPv4(clientIp)) {
            clientIp = '127.0.0.1';
          }

          console.log(`[SOCKET] 🔌 Client connecting from IP: ${clientIp}`);
          console.log(`[SOCKET] 📱 Device info received:`, deviceInfo);

          const device = await DeviceService.registerDevice({
            ...deviceInfo,
            ipAddress: clientIp, // ✅ Use IP from socket connection
            socketId: socket.id
          });
